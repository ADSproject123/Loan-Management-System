...
export async function MembersRequestsContent({
  searchParams,
}: {
  ...
  const { data, count: pendingTotal } = await admin
    .from('members')
    .select('id, full_name, phone, status, role, created_at', { count: 'exact' })
    .eq('is_admin', false)
    .eq('status', 'pending')
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <MembersList members={members} mode="requests" />
